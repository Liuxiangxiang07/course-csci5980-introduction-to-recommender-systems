...
public class SVDModel implements Serializable {
    ...
    private final Long2LongMap userMapping;
    private final Long2LongMap itemMapping;
    ...
    public RealVector getUserVector(long user) {
        long row = userMapping.getOrDefault(user, -1L);
        if (row >= 0) {
    ...
    public RealVector getItemVector(long item) {
        long row = itemMapping.getOrDefault(item, -1L);
        if (row >= 0) {
     ...
     */
    public Long2LongMap getUserIndexMapping() {
        return userMapping;
    }
     ...
     */
    public Long2LongMap getItemIndexMapping() {
        return itemMapping;
    }
    ...
    public int getItemRow(long item) {
        long row = itemMapping.getOrDefault((Long) item, -1L);
        if (row >= 0) {
            return (int) row;
        }
